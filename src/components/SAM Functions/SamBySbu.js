...
class SamBySbu extends Component {
  constructor(props) {
    super(props);
...
setBoxBalue(e){
  ...
  onSbuId(e) {
    ...
    if (e.target.value == "0") {
      ...
      let label = e.target.selectedOptions[0].getAttribute('cname');
      let labelsbu =e.target[e.target.selectedIndex].label
      this.setState({
        sbuId: e.target.value,
        bustatus: false,
        cname:label,
...
// style toast display
    ...
    let optionItems = planets.map(planet => (
      <option key={planet.sbuId} value={planet.sbuId} cname ={planet.cname}>
    ...
    function createHeadersUpper(role) {
        return hrowsUpper.push(<th key={role.roleTitle.titl}>{role.buname}</th>);
      }
    ...
    return (
      ...
      <div className="animated">
       ...
       <CardHeader>
            <i className="icon-menu" />SAM By SBU
